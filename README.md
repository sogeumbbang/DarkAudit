...
| --- | --- | --- | --- |
| Rule Engine 단독 | 0.52 | 1.00 | 0.69 |
| **하이브리드 (LLM 근거 검증 후)** | **1.00** | 0.62 | **0.77** |

Rule Engine은 후보를 넓게 만들어 **미탐 0**을 확보하고, LLM이 각 후보의 근거를 재검증해
확인된 것만 탐지로 보고합니다. 검증 단계에서 **오탐이 0건**이 됩니다.

근거를 확인하지 못한 항목은 조용히 버리지 않습니다. 해당 규칙을 `insufficient_evidence`로
표시해 대시보드의 "검사 범위와 추가 확인 사항"에 **`DA-03: 근거 부족`** 처럼 노출하므로,
검토자가 어떤 규칙이 미확인 상태인지 알고 직접 확인할 수 있습니다.

유형별 하이브리드 성능(3회 평균, `gpt-5.6-luna`)입니다.

| Rule | Precision | Recall | F1 |
| --- | --- | --- | --- |
| `DA-04` 특정옵션의 사전선택 | 1.00 | 1.00 | 1.00 |
| `DA-07` 방해되는 절차 | 1.00 | 1.00 | 1.00 |
| `DA-12` 감정적 언어 | 1.00 | 1.00 | 1.00 |
| `DA-03` 잘못된 계층구조 | — | 0.00 | 0.00 |
| `DA-15` 순차공개 가격책정 | — | 0.00 | 0.00 |

`DA-03`·`DA-15`는 근거 검증을 통과하지 못해 판정에서 제외됐습니다. 두 규칙은 각각 대립
선택지 쌍과 화면 간 가격 비교를 근거로 요구하는데, 합성 스크린샷만으로는 모델이 이를
확정하지 못했습니다. **오탐으로 보고하지 않는 대신 "근거 부족"으로 표시합니다.**

부가 지표: 위치 정확도 IoU 0.5 기준 성공률 **0.82**, 화면 5장 기준 평균 응답 **22.4초**,
스키마 재시도율 15%, Counterfactual 일관성 **0.89**.

### 설계 선택: 확실성과 누락 사이

근거 검증을 완화하면 재현율 1.00을 얻을 수 있으나 오탐이 함께 늘어납니다. 같은 데이터에서
두 설정을 비교하면 이렇습니다.

| 설정 | 검토 대상 | 그중 진짜 | 오탐 | 놓친 문제 |
| --- | --- | --- | --- | --- |
| 검증 완화 | 35건 | 16건 | 19건 | 0건 |
| **현재 (근거 검증)** | **10건** | **10건** | **0건** | 6건 |

현재는 *"확신하는 것만 단정하고, 나머지는 불확실하다고 말한다"* 를 택했습니다. 놓친 항목이
검토자에게 보이지 않는 상태가 아니라 "근거 부족"으로 드러나기 때문입니다. **이 균형은
`evidence_recovery`의 검증 강도로 조정 가능한 설계이며, 운영 정책에 따라 재현율 우선으로
바꿀 수 있습니다.**

### 한계

LLM은 같은 입력에도 다르게 답할 수 있습니다. 위 값은 **3회 측정 평균**이며, 현재 모델에서는
세 번 모두 F1 0.769로 동일했으나 이것이 결정성을 보장하지는 않습니다. Gold Set 기반 2차
라벨링과 라벨러 간 일치도 산출은 아직 하지 않았으므로, 정답 라벨은 생성기가 심은 패턴을
기준으로 합니다.

`docs/eval/hybrid_report.gpt-5.4-mini.json`에 다른 모델의 측정값이 남아 있으나, 근거 검증
도입 이전에 잰 값이라 위 표와 직접 비교할 수 없습니다.
