...
| **하이브리드 (LLM 검증 후)** | **0.46** | **1.00** | **0.63** |

**미탐 0을 유지한 채 정밀도가 0.27에서 0.46으로 올랐습니다.** Rule Engine이 후보를 넓게
...
| `DA-07` 방해되는 절차 | 0.14 | 1.00 | 0.24 |

**MVP 5종 중 4종은 오탐 없이 완벽하고, `DA-07` 하나가 전체 정밀도를 끌어내립니다.**
`DA-07`은 deterministic check가 아직 구현되지 않아(선언 55개 중 11개 구현) 후보가 과하게
생성되며 LLM 검증도 전부 걸러내지 못합니다. **다음 단계의 우선 개선 지점입니다.**

부가 지표: 위치 정확도 IoU 0.5 기준 성공률 **0.94**(mean IoU 0.94), 화면 5장 기준 평균
응답 **13.7초**, 스키마 재시도율 **0%**, Counterfactual 일관성 **0.80**.

### 모델 비교

같은 데이터와 프롬프트로 모델만 바꿔 측정했습니다.

| 모델 | F1 (범위) | Precision | 응답 | 재시도율 | 위치 정확도 |
| --- | --- | --- | --- | --- | --- |
| `gpt-5.4-mini` | 0.57 (0.54~0.58) | 0.40 | 5.9초 | 8% | 1.00 |
| **`gpt-5.6-luna`** | **0.63 (0.63~0.63)** | **0.46** | 13.7초 | **0%** | 0.94 |

`gpt-5.6-luna`는 `DA-04`를 0.35에서 1.00으로 끌어올렸고 **3회 측정이 모두 동일**해 회차 간
변동이 없습니다. 대신 응답이 2.3배 느리고 위치 정확도가 소폭 낮습니다. 재현성과 정밀도를
우선해 이 모델을 사용합니다. 비교는 `python backend/compare_eval.py`로 재현합니다.

### 한계

LLM은 같은 입력에도 다르게 답할 수 있습니다. 위 값은 **3회 측정 평균**이며, 현재 모델에서는
세 번 모두 F1 0.627로 동일했으나 이것이 결정성을 보장하지는 않습니다. Gold Set 기반 2차
